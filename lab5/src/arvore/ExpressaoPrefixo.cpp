...
Valor *ExpressaoPrefixo::calcula_valor(vector<Valor *> &params)
{
	if (operador == "!")
	{
		...
		if (val->tipo == TIPO_REF_INT || val->tipo == TIPO_REF_FLOAT || val->tipo == TIPO_REF_BOOL)
		{
...
void ExpressaoPrefixo::checa_variaveis(bool chamada)
{
	expressao->checa_variaveis(chamada);
}
void ExpressaoPrefixo::carrega_frame(FrameFuncao *frame)
{
	expressao->carrega_frame(frame);
}
Stm *ExpressaoPrefixo::cria_stm()
{
	return expressao->cria_stm();
}

Exp *ExpressaoPrefixo::cria_exp()
{
	return expressao->cria_exp();
}
