...
Exp *ExpressaoChamadaFuncao::cria_exp()
{
	ExpName *name = ExpName::gera_Exp_Name(nome_funcao->valor);
	vector<Exp *> parametros;
	for (int i = 0; i < argumentos.size(); i++)
	{
		parametros.push_back(argumentos[i]->cria_exp());
	}

	ExpCall *call = ExpCall::gera_Exp_Call(name, parametros);

	return call;
}

Stm *ExpressaoChamadaFuncao::cria_stm()
{
}
