...
ExpressaoSoma::ExpressaoSoma(No_arv_parse *no)
{
	esquerda = Expressao::extrai_expressao(no->filhos[0]);
	direita = Expressao::extrai_expressao(no->filhos[2]);
}

Valor *ExpressaoSoma::calcula_valor(vector<Valor *> &params)
{
	...
	if (val1->tipo == val2->tipo)
	{
		...
		{
			cerr << "Tipos inválidos na Soma" << endl;
		}
	}
	else
	{
...
void ExpressaoSoma::checa_variaveis(bool chamada)
{
	esquerda->checa_variaveis(chamada);
	direita->checa_variaveis(chamada);
}
void ExpressaoSoma::carrega_frame(FrameFuncao *frame)
{
	esquerda->carrega_frame(frame);
	direita->carrega_frame(frame);
}

Exp *ExpressaoSoma::cria_exp()
{
