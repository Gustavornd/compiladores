...
ExpressaoDiferente::ExpressaoDiferente(No_arv_parse *no) {
  this->esquerda = Expressao::extrai_expressao(no->filhos[0]);
  this->direita = Expressao::extrai_expressao(no->filhos[2]);
}

Valor *ExpressaoDiferente::calcula_valor(vector<Valor *> &params) {
  ...
  if (val1->tipo == val2->tipo) {
    ...
    if (val1->tipo == TIPO_INT) {
      ...
      cerr << "Erro: Diferente (!=) com tipos invalidos" << endl;
      return nullptr;
    }
    return resultado;
  }
