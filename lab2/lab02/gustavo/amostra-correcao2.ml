(* Tradução C -> OCaml
   - Structs foram convertidas em argumentos independentes (ex: x e y separados).
   - Loops for foram substituídos por loops while com referências.
   - Variáveis mutáveis utilizam refs (! para acessar, := para atribuir).
   - Funções com nomes idênticos no C foram sufixadas para evitar colisão. 
*)
...
let rec func vx vy n tax tay tbx tby tcx tcy tcor =
  if n <= 0 then
    ...
    let i = ref (n - 1) in
    while !i >= 0 && vx.(!i) > 0 do
      let temp = float_of_int ((vy.(!i) * vx.(!i)) mod 123) in
      if temp < 0.0 then
...
let f2_a tax tay tbx tby tcx tcy tcor =
  ...
  if (tax >= 10 || tby > 20 || tay < 30 || tbx <= 50) && not (tcx <> 90 || tcy = 0) then
    ...
    while !a < 10 do
      ...
      total := !total + (tbx * tay);
      total := !total + (tax * tby);
      soma.(!a) <- float_of_int (!total mod 100);
      a := !a + 1
    done;
    0 (* Valor de retorno padrão (o código original em C não possuía retorno no else) *)
  end
...
let area_poligono px_arr py_arr n =
  ...
  while !i < n do
    let j = (!i + 1) mod n in
    soma := !soma + (px_arr.(!i) * py_arr.(j)) - (px_arr.(j) * py_arr.(!i));
    i := !i + 1
  done;
...
let area_pol () =
  let poli_x = Array.make 1000 0 in
  let poli_y = Array.make 1000 0 in
